...
/** Compact stake selector: −/+ stepper, quick chips, min/max clamp. */
  ...
  return (
    <div className={disabled ? 'pointer-events-none opacity-60' : ''}>
      <div className="flex items-center gap-2">
        <button
          onClick={() => step(-10)}
          className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-white/10 bg-white/5 text-white active:scale-90"
        >
          <Minus size={18} />
        </button>
        <div className="flex flex-1 items-center justify-center gap-1.5 rounded-2xl border border-teal-400/25 bg-teal-500/[0.06] py-2.5">
      ...
      <div className="mt-2 grid grid-cols-5 gap-2">
        {CHIPS.map((c) => (
          <button
            ...
            className={`rounded-xl border py-2 text-xs font-bold transition active:scale-95 ${
              value === c ? 'border-teal-400/40 bg-teal-500/15 text-teal-200' : 'border-white/10 bg-white/[0.03] text-slate-300'
            }`}
          >
            {c}
          </button>
        ...
        <button
          ...
          className="rounded-xl border border-white/10 bg-white/[0.03] py-2 text-xs font-bold text-slate-300 transition active:scale-95"
        >
          Max
        </button>
      </div>
